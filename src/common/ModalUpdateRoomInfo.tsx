...
import { closeModalAddMember,closeModalUpdateRoomInfo,oppenModalAddMember } from "../redux/statusCommon/slice";
    ...
    async function handleUpdate(){
        try {
            ...
            var room: any = await axios.patch(
                `http://localhost:5000/api/rooms/${roomId}/name`,data,
                {
            ...
            dispatch(closeModalUpdateRoomInfo())
            dispatch(roomAPI.saveRoomId()({_id:roomId,avatar:preview,name:name}))
            dispatch(userAPI.updateRoomByIdUI()({_id:roomId,avatar:preview,name:name}))
        ...
        <div>
            ...
            >
                <div className={style.Modal_createGroup}>
                    ...
                    <div className={style.UpdateAvatart} style={{display:"flex", margin:"10px"}}>
                        <div className="" style={{ margin:"auto"}} onClick={clickUploadFile}>
                            <img style={{width:"60px", height:"60px", borderRadius:"50%"}} src={preview} alt="" />
                            <AiFillEdit />
                            <input
                                hidden
                                ref={fileInput}
                    ...
                    <div className="" style={{fontSize:"14px"}}>Bạn có chắc muốn thây đổi thông tin cuộc hội thoại, khi xác nhận thông tin hội thoại mới sẽ hiển thị với tất cả thành viên</div>

                    <div className={style.search_menber_block} style={{border:"1px solid #e5e7eb", borderRadius:"3px"}}>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div className={style.Modal_footer} style={{border:"none"}}>
                        <button className={style.btn_modal_cancel} onClick={close}>Hủy</button>                       
                            <button
